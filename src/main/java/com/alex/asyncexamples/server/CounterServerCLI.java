...
public class CounterServerCLI {
	public static void main(String args[]) throws IOException {
		...
		while (running) {
			...
			case "start":
				ServerManager.INSTANCE.startServer(serverName, InetAddress
						.getLocalHost().getHostName(), Integer
						.parseInt(serverPortNumber));
				break;
			case "stop":
			...
			case "restart":
				...
				break;
			case "list":
				List<String> servers = ServerManager.INSTANCE
						.listServersRunning();
			...
			case "help":
				...
				break;
			default:
				returnMessage = "Did not recognise input: " + input
						+ ", try again.";
				break;
			}
	...
	private static void showHelp() {
		...
		System.out
				.println("start %servername% %port% -- start server on a port e.g. start srv1 9999  ");
		System.out.println("stop %servername% -- stop server e.g. stop srv1");
		System.out.println("list -- list all servers running");
		System.out
				.println("restart %servername% -- restart %servername% e.g. restart srv1");
