 ...
 * fixed sized thread pool.</LI>
 * Several mitigation strategies for this. the one I used was to use a
 * CachedThreadPool which would increase in pool size as needs be. There are
 * also read / write timeout set in the callback logic. To make this server even
 * more sophisticated logic could be added so that if a counter has been changed
 * and then the write back to the client fails, the change rolls back.
 * <LI>Too many reads happening at the same time and overwhelming memory</LI>
 * A mitigation strategy here would be to throttle using a pool of ByteBuffers.
 * I didn't implement this in this in this example as according to Alan
 * Bateman's JavaOne presentation it was possible to reach 10,000 connection
...
public final class AsyncEchoServer implements Runnable {
	...
	AsyncEchoServer(String name) throws IOException, InterruptedException,
			ExecutionException {
		this.name = name;
		asyncChannelGroup = AsynchronousChannelGroup.withThreadPool(Executors
	...
	void open(InetSocketAddress serverAddress) throws IOException {
		...
		asyncServerSocketChannel = AsynchronousServerSocketChannel.open(
				asyncChannelGroup).bind(serverAddress);
		asyncServerSocketChannel.setOption(StandardSocketOptions.SO_RCVBUF,
				MESSAGE_INPUT_SIZE);
		asyncServerSocketChannel.setOption(StandardSocketOptions.SO_REUSEADDR,
				true);
	}
	...
	private void handleAcceptConnection(
			AsynchronousSocketChannel asyncSocketChannel) {
		logger.info(">>handleAcceptConnection(), asyncSocketChannel="
				+ asyncSocketChannel);
		...
		try {
			...
			Future<Integer> futureReadResult = asyncSocketChannel
					.read(messageByteBuffer);
			...
			Future<Integer> futureWriteResult = asyncSocketChannel
					.write(messageByteBuffer);
